...
public class MeshCombiner : MonoBehaviour
{
	...
	public MeshFilter[] MeshFiltersToSkip { get { return meshFiltersToSkip; } set { meshFiltersToSkip = value; } }
	...
	/// </summary>
	public Mesh CombineMeshes(bool showCreatedMeshInfo, MeshFilter[] meshFilters = null, Mesh existingMesh = null)
	{
		Mesh generatedMesh;
		...
		if (!createMultiMaterialMesh)
			generatedMesh = CombineMeshesWithSingleMaterial(showCreatedMeshInfo, GetMeshFilters(), existingMesh);
		else
			generatedMesh = CombineMeshesWithMutliMaterial(showCreatedMeshInfo, GetMeshFilters(), existingMesh);
		...
		MeshFilter[] GetMeshFilters()
		{
			MeshFilter[] newValues;

			if (meshFilters != null)
			{
				newValues = new MeshFilter[meshFilters.Length + 1];
				newValues[0] = GetComponent<MeshFilter>();
				Array.Copy(meshFilters, 0, newValues, 1, meshFilters.Length);
			}
			else
				newValues = GetMeshFiltersToCombine();
	...
	private MeshFilter[] GetMeshFiltersToCombine()
	{
		...
		//	if (meshFiltersToSkip.FirstOrDefault((m) => m == mf) == null)
		//		meshFiltersList.Add(mf);
		//}

		//return meshFiltersList.ToArray();
		return meshFilters;
	}

	private Mesh CombineMeshesWithSingleMaterial(bool showCreatedMeshInfo, MeshFilter[] meshFilters, Mesh existingMesh)
	{
	...
	private Mesh CombineMeshesWithMutliMaterial(bool showCreatedMeshInfo, MeshFilter[] meshFilters, Mesh existingMesh)
	{
		...
		for (int i = 0; i < meshFilters.Length - 1; i++)
		{
			...
			if (meshRenderers[i + 1] != null)
			{
				...
				foreach (Material t in materials)
					if (!uniqueMaterialsList.Contains(t)) // If Material doesn't exists in the list then add it.
						uniqueMaterialsList.Add(t);
			}
		}
		...
		foreach (var sbm in finalMeshCombineInstancesList)
		{
			sbm.mesh.Clear();
#if UNITY_EDITOR
			DestroyImmediate(sbm.mesh);
#else
			Destroy(sbm.mesh);
#endif
		}
